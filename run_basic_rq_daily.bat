...
REM 与本 bat 同目录须有所列 .py；交易日读 Mongo economic.trade_dates
REM =============================================================================
...
"%PYTHON%" "%~dp0rq_daily_update\update_rqbaseInfo.py"
if errorlevel 1 (
    echo.
    echo [1/3] 失败，停止。
    pause
...
"%PYTHON%" "%~dp0rq_daily_update\update_rq_basic_financail.py"
if errorlevel 1 (
    echo.
    echo [2/3] 失败，停止。
    pause
...
"%PYTHON%" "%~dp0rq_daily_update\update_rq_in_index.py"
if errorlevel 1 (
    echo.
    echo [3/3] 失败，停止。
    pause
