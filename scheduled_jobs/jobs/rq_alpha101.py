# -*- coding: utf-8 -*-
"""调用 rq_daily_update/update_rq_alpha101（写入上一交易日）。"""
...
SCHEDULER_JOB_KEY = "rq_alpha101"


def run() -> JobResult:
    from rq_paths import bootstrap

    bootstrap(str(_ROOT / "rq_daily_update" / "update_rq_alpha101.py"))
    from trade_date_utils import is_trade_day, now_shanghai, previous_trade_date, today_shanghai
    from rq_daily_update.update_rq_alpha101 import update_rq_alpha101
    ...
    try:
        ok = update_rq_alpha101(target, mongo_alias=mongo_alias)
    except RuntimeError as e:
        return JobResult(
            job_id=SCHEDULER_JOB_KEY,
            ok=False,
            skipped=False,
            message=f"流量中止或运行异常：{e}",
            detail={
                "run_at": run_at,
                "today": today,
                "target_date": target,
                "collection": "basic_rq.rq_alpha101",
            },
        )
