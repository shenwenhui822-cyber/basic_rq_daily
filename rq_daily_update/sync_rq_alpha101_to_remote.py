# -*- coding: utf-8 -*-
"""
将 basic_rq.rq_alpha101 按交易日同步到远端 MongoDB。

源：wonderwz27018_rw @ 192.168.110.199:27018
目标：wonderwz203_19_rw @ 114.80.62.203:27019

用法（basic_rq_daily 根目录）：
  python rq_daily_update/sync_rq_alpha101_to_remote.py --date 2026-09-15
  python rq_daily_update/sync_rq_alpha101_to_remote.py --start 2026-09-01 --end 2026-09-15
  python rq_daily_update/sync_rq_alpha101_to_remote.py
"""
...
def sync_rq_alpha101_for_dates(
    ...
    *,
    ...
    for trade_date in trade_dates:
        _log(f"\n=== 同步 rq_alpha101 {trade_date} ===")
        stats = sync_alpha101_for_date(
            src_col=src_col,
            dst_col=dst_col,
        ...
        if stats["source"] == 0:
            errors.append(f"{trade_date}: 源库无数据")
...
def sync_rq_alpha101_t_minus_one(
    *,
    source_alias: str = DEFAULT_SOURCE_ALIAS,
    target_alias: str = DEFAULT_TARGET_ALIAS,
    ...
    target = previous_trade_date(mongo_alias=alias, fmt="%Y-%m-%d")
    return sync_rq_alpha101_for_dates(
        [target],
        source_alias=source_alias,
        target_alias=target_alias,
    )
...
def _cli_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="同步 basic_rq.rq_alpha101 到远端 wonderwz203_19_rw"
    )
    ...
    p.add_argument(
        ...
        help=f"源 Mongo 别名，默认 {DEFAULT_SOURCE_ALIAS}（199）",
    )
    p.add_argument(
        "--target-alias",
        default=DEFAULT_TARGET_ALIAS,
        help=f"目标 Mongo 别名，默认 {DEFAULT_TARGET_ALIAS}（203）",
    )
    p.add_argument(
        "--mongo-trade-alias",
        default=None,
...
def main() -> int:
    ...
    try:
        if args.start or args.end:
            ...
            result = sync_rq_alpha101_for_dates(
                dates,
            ...
            result = sync_rq_alpha101_for_dates(
                [str(args.date).strip().replace("/", "-")[:10]],
                source_alias=args.source_alias,
                target_alias=args.target_alias,
            )
        else:
            result = sync_rq_alpha101_t_minus_one(
                source_alias=args.source_alias,
                target_alias=args.target_alias,
