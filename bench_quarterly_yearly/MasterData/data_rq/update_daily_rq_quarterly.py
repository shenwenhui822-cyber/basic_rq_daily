"""
每日更新 + 按票补齐 basic_rq.rq_quarterly。

执行内容：
1) 先更新 pre_trade_day 当日 rq_quarterly（米筐 PIT）
...
用法（项目根目录，PYTHONPATH=.）：
  ...
  python -u MasterData/data_rq/update_daily_rq_quarterly.py --run-date 2026-04-20
  python -u MasterData/data_rq/update_daily_rq_quarterly.py --auto-mode previous_trade
  python -u MasterData/data_rq/update_daily_rq_quarterly.py --pre-day 2026-04-17
  python -u MasterData/data_rq/update_daily_rq_quarterly.py --backfill-start 2020-01-02
  python -u MasterData/data_rq/update_daily_rq_quarterly.py --dry-run
"""
...
def ensure_indexes_quarterly(client, mongo_db: str, coll_name: str = "rq_quarterly") -> None:
...
def run_backfill_quarterly(
    *,
    ...
    print(f"[backfill] rq_quarterly 逐票补齐区间：{trade_list[0]} ~ {trade_list[-1]}（{len(trade_list)} 个交易日）")
    db = client[mongo_db]
...
def main() -> None:
    p = argparse.ArgumentParser(description="每日更新 + 按票补齐 rq_quarterly")
    ...
    if not today_for_update:
        raise SystemExit(f"无法找到 {pre_trade_day} 的下一交易日，update_rq_quarterly 无法执行")
    ...
    if args.dry_run:
        print(f"[dry-run] 将执行 update_rq_quarterly(pre={pre_trade_day}, today={today_for_update})")
        if args.no_backfill:
            print("[dry-run] no-backfill=true，跳过 backfill")
        else:
            print(f"[dry-run] 将执行 rq_quarterly 逐票 backfill: {backfill_start} ~ {backfill_end}")
        return

    if not args.no_index:
        try:
            ensure_indexes_quarterly(client, args.mongo_db, "rq_quarterly")
